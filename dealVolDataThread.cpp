...
{ 
	...
	if (RunState::insert == m_iJobflag)
	{
		statement = CGlobalDataSaver::GetInstance()->m_pTextCode->toUnicode("%1 INTO E_MP_VOL_CURVE_P (vol_cur_id, meter_id, phase, data_date, metering_time,"
			...
			rc = SQLSetStmtAttr(hstmt_dre, SQL_ATTR_PARAM_BIND_TYPE, (SQLPOINTER) sizeof(crm_mnp_npdb_esvf), 0);
			if (!SQL_SUCCEEDED(rc))
			{
				sErrorMsg = diagnostic(SQL_HANDLE_STMT, &hstmt_dre);
			...
			rc = SQLBindParameter(hstmt_dre, 3, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, po[0].len_phase, 0, po[0].phase, sizeof(po[0].phase), &po[0].len_phase);
			rc = SQLBindParameter(hstmt_dre, 4, SQL_PARAM_INPUT, SQL_C_TYPE_TIMESTAMP, SQL_TIMESTAMP, po[0].len_data_date, 0, &po[0].data_date, sizeof(po[0].data_date), &po[0].len_data_date);
			rc = SQLBindParameter(hstmt_dre, 5, SQL_PARAM_INPUT, SQL_C_TYPE_TIMESTAMP, SQL_TIMESTAMP, po[0].len_metering_time, 0, &po[0].metering_time, sizeof(po[0].metering_time), &po[0].len_metering_time);
			...
			rc = SQLBindParameter(hstmt_dre, 101, SQL_PARAM_INPUT, SQL_C_DOUBLE, SQL_DOUBLE, po[0].len_V96, 0, &po[0].V96, sizeof(po[0].V96), &po[0].len_V96);
			...
			m_iCommitedLine = m_iCurrlLine;
			m_iCommitedSize = (LONG64)((m_iCurrlLine * (sizeof(crm_mnp_npdb_esvf) + sizeof(SQLUSMALLINT))) / (1024 * 1024));
